...
var CODIUS_CONFIG_FILENAME = 'codius-config.json';
...
var Compiler = function (config) {
  ...
  this.config = config;
  this.ignoreFiles = config.ignoreFiles || ['.codiusignore', '.gitignore', '.npmignore'];
...
Compiler.prototype._ensureFilesProperty = function (manifest, contractPath) {
  ...
  if (!manifest.files) {
    ...
    manifest.files = _this._findAllFiles(contractPath, [_this.config.manifestFilename, 'node_modules', '.git', CODIUS_CONFIG_FILENAME]);
...
Compiler.prototype._ensureEnvProperty = function (manifest, contractPath) {
  ...
  var configPath = path.join(contractPath, CODIUS_CONFIG_FILENAME);
  var config;
