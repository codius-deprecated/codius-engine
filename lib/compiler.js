 ...
 */
Compiler.prototype.compileModule = function (contract_dir) {
  var _this = this;

  if (!fs.existsSync(_this.config.contractsFilesystemPath)) {
    fs.mkdirSync(_this.config.contractsFilesystemPath);
  }

  // Read existing manifest or create a new one
  var manifest;
  var manifest_path = path.join(contract_dir, _this.config.manifestFilename);
  ...
  var files = _this.findAllFiles(contract_dir, [_this.config.manifestFilename, 'contract_modules']);
