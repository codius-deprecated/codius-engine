...
Compiler.prototype.compileModule = function (contractPath) {
  ...
  _this.ensureMainProperty(manifest, contractPath);
  _this.ensureFilesProperty(manifest, contractPath);
  _this.ensureModulesProperty(manifest, contractPath);
  _this.ensureApisProperty(manifest);
...
Compiler.prototype.ensureMainProperty = function (manifest, contractPath) {
  var _this = this;
...
Compiler.prototype.ensureFilesProperty = function (manifest, contractPath) {
  var _this = this;

  if (!manifest.files) {
    // Go through all files and subdirectory files
    // excluding the node_modules
    manifest.files = _this.findAllFiles(contractPath, [_this.config.manifestFilename, 'node_modules']);
  ...
  manifest.files = _this.expandFilesProperty(manifest.files, contractPath);

  // TODO Strip off initial slashes
};

Compiler.prototype.expandFilesProperty = function (files, filepath) {
  var self = this;
  ...
  if (Array.isArray(files)) {
    ...
    return self.expandFilesProperty(fileHashMap, filepath);
...
Compiler.prototype.ensureModulesProperty = function (manifest, contractPath) {
  var _this = this;
...
Compiler.prototype.ensureApisProperty = function (manifest, contractPath) {
  var _this = this;
